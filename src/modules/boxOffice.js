...
const GET_MOVIE_TMP = 'GET_MOVIE_TMP';
...
export default handleActions({
    ...pender({
        type: GET_MOVIE_TMP, // type 이 주어지면, 이 type 에 접미사를 붙인 액션핸들러들이 담긴 객체를 생성합니다.
        /*
    ...
    [GET_BOX_OFFICE]: (state, action) => {
        const test = boxOffice.getBoxOiffceList();

        return {
            items: state.items,
            boxOfficeList: test[0].cards
        }
    }
}, initialState);
